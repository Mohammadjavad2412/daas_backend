...
class Desktop:
    ...
    def create_daas(self,email,password,http_port=None,https_port=None):
        if not http_port and not https_port:
            ...
            subprocess.call(['docker','run','-d','-e','TITLE=net-sep'#,'-e',f'CUSTOM_USER={email}','-e',f'PASSWORD={password}'
                             ,'-p',f"{http_port}:3000",'-p',f"{https_port}:3001",'lscr.io/linuxserver/webtop:ubuntu-kde'])
